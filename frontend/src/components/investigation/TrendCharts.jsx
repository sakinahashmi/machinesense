...
export default function TrendCharts({ trends = [] }) {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white border border-[#D8E0E6] p-2.5 rounded shadow-sm text-xs font-mono text-slate-800">
          ...
          {data.dimensional_deviation !== null && (
            <p className="text-[#C34B4B] font-semibold">Deviation: +{data.dimensional_deviation} mm</p>
          )}
          ...
          {data.is_failure_point && (
            <p className="mt-1 text-[#C34B4B] font-bold border-t border-slate-200 pt-1">★ Non-Conformance Point (CNC-2847)</p>
          )}
        </div>
      );
    }
        ...
        <ResponsiveContainer width="100%" height="100%">
          {activeTab === 'deviation' ? (
            <LineChart data={trends} margin={{ top: 10, right: 20, left: -10, bottom: 0 }}>
              ...
              <XAxis dataKey="timestamp" stroke="#667685" tick={{ fontSize: 10, fontFamily: 'monospace' }} />
              <YAxis stroke="#667685" tick={{ fontSize: 10, fontFamily: 'monospace' }} domain={[0, 0.5]} />
              <Tooltip content={<CustomTooltip />} />
              <ReferenceLine y={0.10} stroke="#C34B4B" strokeDasharray="3 3" label={{ value: 'Upper Tol Limit (+0.10 mm)', fill: '#C34B4B', fontSize: 10, position: 'top' }} />
              <Line
                type="monotone"
                ...
                dot={(props) => {
                  ...
                  return isFail ? (
                    <circle cx={props.cx} cy={props.cy} r={5} fill="#C34B4B" stroke="#FFFFFF" strokeWidth={2} key={props.key} />
                  ) : (
                    <circle cx={props.cx} cy={props.cy} r={2} fill="#356B7A" key={props.key} />
                  );
                }}
              />
            </LineChart>
            ...
            <LineChart data={trends} margin={{ top: 10, right: 20, left: -10, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#D8E0E6" vertical={false} />
              <XAxis dataKey="timestamp" stroke="#667685" tick={{ fontSize: 10, fontFamily: 'monospace' }} />
              <YAxis stroke="#667685" tick={{ fontSize: 10, fontFamily: 'monospace' }} domain={[0, 4.5]} />
              <Tooltip content={<CustomTooltip />} />
              <ReferenceLine y={2.0} stroke="#C34B4B" strokeDasharray="3 3" label={{ value: 'Vibration Limit (2.0 mm/s)', fill: '#C34B4B', fontSize: 10 }} />
            ...
            <LineChart data={trends} margin={{ top: 10, right: 20, left: -10, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#D8E0E6" vertical={false} />
              <XAxis dataKey="timestamp" stroke="#667685" tick={{ fontSize: 10, fontFamily: 'monospace' }} />
              <YAxis stroke="#667685" tick={{ fontSize: 10, fontFamily: 'monospace' }} domain={[3.5, 5.8]} />
              <Tooltip content={<CustomTooltip />} />
              <ReferenceLine y={4.6} stroke="#C34B4B" strokeDasharray="3 3" label={{ value: 'Nominal Baseline Limit (4.60 kW)', fill: '#C34B4B', fontSize: 10 }} />
              <Line type="monotone" dataKey="power_consumption" stroke="#356B7A" strokeWidth={2} dot={false} />
            </LineChart>
          ) : (
            <LineChart data={trends} margin={{ top: 10, right: 20, left: -10, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#D8E0E6" vertical={false} />
              <XAxis dataKey="timestamp" stroke="#667685" tick={{ fontSize: 10, fontFamily: 'monospace' }} />
              <YAxis stroke="#667685" tick={{ fontSize: 10, fontFamily: 'monospace' }} domain={[30, 50]} />
              <Tooltip content={<CustomTooltip />} />
              <ReferenceLine y={42.0} stroke="#C34B4B" strokeDasharray="3 3" label={{ value: 'Thermal Warning (42°C)', fill: '#C34B4B', fontSize: 10 }} />
              <Line type="monotone" dataKey="temperature" stroke="#23805A" strokeWidth={2} dot={false} />
            </LineChart>
          )}
        </ResponsiveContainer>
      </div>
